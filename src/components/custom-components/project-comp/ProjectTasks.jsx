...
import { PlayCircle, BookOpen, Code2, UploadCloud, Layers } from "lucide-react";
...
export default function ProjectTasks({ project }) {
  const [activeTaskId, setActiveTaskId] = useState(project.tasks[0]._id);
  ...
  return (
    <>
      {!isTaskDetail ? (
        ...
        <ProjectTaskDetail
          task={taskDetail}
          setIsTaskDetail={setIsTaskDetail}
        />
      )}
    </>
  );
